...
const ProfileData = (props) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            about: Yup.string().min(3, 'Info must contain more than 3 characters!'),
            jobDescription: Yup.string().min(3, 'Job description must contain more than 3 characters!').required(),
            vk: urlError,
            facebook: urlError,
            instagram: urlError,
            twitter: urlError,
    ...
    const toggleProfileDataEditMode = (editMode, setEditMode) => {
        if (editMode === false && isCurrentUser === true) {
            setEditMode(true)
        } else if (editMode === true && !formik.errors.name && !formik.errors.about) {
            setEditMode(false)
            props.updateProfileTC(currentUser, aboutInfo, (jobInfo !== "enter job description"), jobInfo, formik.values.name, githubLink, vkLink, facebookLink, instagramLink,
                twitterLink, websiteLink, youtubeLink, mainLink
            )
        }
    }
    ...
    window.isLook = formik.values.isLookingForAJob
    window.description = formik.values.jobDescription
    window.jobInfo = jobInfo
        ...
        <form className="profile-page-right-part">
            ...
            <div className={"profile-page-job-info-block"}>
                ...
                {jobEditMode ? <input className="profile-page-input"
                                      ...
                                      onChange={formik.handleChange}/>
                    :
                    <p onDoubleClick={() => toggleProfileDataEditMode(jobEditMode, setJobEditMode)}>{formik.values.jobDescription === "" && isCurrentUser || formik.values.jobDescription === "enter job description" && isCurrentUser ? <span><AiOutlinePlusSquare/>Click to add job description</span> : formik.values.jobDescription}</p>}
